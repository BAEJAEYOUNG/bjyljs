...
//    colModel.push({label:"상품코드", name:"prodMclsCd", width:80});
    ...
    group.groupHeaders.push({startColumnName: 'userCntT', numberOfColumns: 3, titleText: "합계 (선불+후불)"});
...
function doQuery() {
 ...
 // search-panel 의 form name, value를 <k,v> 형태로 params 에 담는다.
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/salestat/salestatday/list"
                      , params
                      , function(result) {

        if(result.resultCd == '00') {
            setGridHeader();
            CommonJs.setStatus(  ksid.string.replace( "일별 매출현황 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
function doExcel() {
    ...
    var url = "${pageContext.request.contextPath}/salestat/salestatday/excel";
    var fileNm = "일별매출현황목록";
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">매출현황</span> > <span name="menu">일별 매출현황</span></p>
    </div>
    ...
    <h3 class="style-title"><span name="title">일별 매출현황 목록</span></h3>
    <!-- grid -->
