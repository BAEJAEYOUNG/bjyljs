...
function init() {
    ...
    // PG사 지불수단 정산현황 그리드
    var colModel = [];
    colModel.push({label:"지급일자", name:"payDay", format:"date", width:80});
...
//    colModel.push({label:"상품코드", name:"prodMclsCd", width:80});
    colModel.push({label:"상품명", name:"prodMclsNm", width:70});
    colModel.push({label:"지불수단", name:"payMethod", width:60});
    colModel.push({label:"사용자수", name:"userCnt", format:"number", width:60});
    colModel.push({label:"건수", name:"apprCnt", format:"number", width:70});
    colModel.push({label:"금액", name:"apprAmt", format:"number", width:100});
    colModel.push({label:"수수료", name:"apprCharge", format:"number", width:80});
    colModel.push({label:"부가세(VAT)", name:"apprVat", format:"number", width:80});
    colModel.push({label:"건수", name:"cancelCnt", format:"number", width:70});
    colModel.push({label:"금액", name:"cancelAmt", format:"number", width:100});
    colModel.push({label:"수수료", name:"cancelCharge", format:"number", width:80});
    colModel.push({label:"부가세(VAT)", name:"cancelVat", format:"number", width:80});
    colModel.push({label:"건수", name:"totalCnt", format:"number", width:70});
    colModel.push({label:"금액", name:"totalAmt", format:"number", width:100});
    colModel.push({label:"수수료", name:"totalCharge", format:"number", width:80});
    colModel.push({label:"부가세(VAT)", name:"totalVat", format:"number", width:80});
    colModel.push({label:"정산금액", name:"settleAmt", format:"number", width:130});
    ...
    gridProp.loadComplete = function() {
        ...
        var option = {};
        option.col = [ "userCnt"
                     , "apprCnt", "apprAmt", "apprCharge", "apprVat"
                     , "cancelCnt", "cancelAmt", "cancelCharge", "cancelVat"
                     , "totalCnt", "totalAmt", "totalCharge", "totalVat", "settleAmt"
                       ];
        option.label = { "payDay" : "합계" };
        grid1.setSum(option);
        ...
        for (var i = 0; i < gridData.length; i++) {
            //ksid.ui.alert("totalAmt :" + gridData[i].totalAmt);
            if (gridData[i].totalAmt < 0) {
                $('#grid1').jqGrid('setCell', ids[i], 'totalAmt', '', cssOrange);
            }
            if (gridData[i].settleAmt < 0) {
                $('#grid1').jqGrid('setCell', ids[i], 'settleAmt', '', cssOrange);
            }
        }
    };

    // PG사 지불수단 정산현황 그리드 생성
    ...
    group.groupHeaders = [];
    group.groupHeaders.push({startColumnName: 'apprCnt', numberOfColumns: 4, titleText: "결제승인 내역 (수수료는 VAT포함)"});
    group.groupHeaders.push({startColumnName: 'cancelCnt', numberOfColumns: 4, titleText: "결제취소 내역 (수수료는 VAT포함)"});
    group.groupHeaders.push({startColumnName: 'totalCnt', numberOfColumns: 4, titleText: "결제 합계 내역 (수수료는 VAT포함)"});
    group.groupHeaders.push({startColumnName: 'settleAmt', numberOfColumns: 1, titleText: "(금액 - 수수료)"});
    ...
    var sDate = new ksid.datetime().before(0,1,8).getDate("yyyy-mm-dd");
    var eDate = new ksid.datetime().before(0,0,8).getDate("yyyy-mm-dd");
...
function doQuery() {
 ...
 // search-panel 의 form name, value를 <k,v> 형태로 params 에 담는다.
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/salestat/settlepgstatday/list"
                      , params
                      , function(result) {

        if(result.resultCd == '00') {
            setGridHeader();
            CommonJs.setStatus(  ksid.string.replace( "PG사 지불수단 정산현황 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
function doExcel() {
    ...
    var url = "${pageContext.request.contextPath}/salestat/settlepgstatday/excel";
    var fileNm = "PG사지불수단정산현황";
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">매출현황</span> > <span name="menu">PG사 지불수단 정산현황</span></p>
    </div>
    ...
    <h3 class="style-title"><span name="title">PG지불수단별 정산현황 목록(지급기준:7영업일 후)</span></h3>
    <!-- grid -->
