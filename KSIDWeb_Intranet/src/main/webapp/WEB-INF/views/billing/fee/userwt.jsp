...
// 사용자 FIDO 요금 데이터정보 json
var newJson1 = {};

// 사용자 FIDO 요금 데이터리스트 grid
var grid1 = null;
...
//    colModel.push({ label: '인증수단', name: 'authMethodNm', width:60 });
...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/fee/userwt/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus(  ksid.string.replace( "사용자 FIDO 요금 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
/*****************************************************
* 함수명: 사용자 FIDO 요금 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/fee/userwt/excel";
    var fileNm = "사용자FIDO요금데이터";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">요금조회</span> > <span name="menu">사용자 FIDO 요금</span></p>
    </div>
...
<!--                     <select name="eYymm" title="종료월" class="style-select" codeGroupCd="yyyy_mm"></select> -->
    ...
    <h3 class="style-title"><span name="title">사용자 FIDO 요금 데이터 목록</span></h3>
    <!-- grid -->
