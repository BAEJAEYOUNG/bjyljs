...
// 일자별 PG사 정산대사 수집 데이터정보 json
var newJson1 = {};

// 일자별 PG사 정산대사 수집 데이터리스트 grid
var grid1 = null;
...
function init() {
    ...
    var colModel = [];
    colModel.push({ label: '정산일자', name: 'payDtm', format: 'date', width:70 });
    colModel.push({ label: '거래ID', name: 'payTid', width:220 });
    colModel.push({ label: '가맹점 ID', name: 'payMid', width:80 });
    colModel.push({ label: '승인일시', name: 'apprDtm', format: 'dttm', width:120 });
    colModel.push({ label: '취소일시', name: 'cancelDtm', format: 'dttm', width:120 });
    colModel.push({ label: '거래구분', name: 'payCmdNm', width:70 });
    colModel.push({ label: '결제수단', name: 'payMethodNm', width:70 });
    colModel.push({ label: '주문번호', name: 'payOid', width:120 });
    colModel.push({ label: '승인번호', name: 'authNo', width:70 });
    colModel.push({ label: '결제금액', name: 'payAmt', format: 'number', width:70 });
    colModel.push({ label: '무이자구분', name: 'freeFgNm', width:70 });
    colModel.push({ label: '수수료(VAT)', name: 'charge', format: 'number', width:80 });
    colModel.push({ label: '부가세', name: 'vatFee', format: 'number', width:60 });
    colModel.push({ label: '지급일자', name: 'payDay', format: 'date', width:70 });
    colModel.push({ label: '카드사', name: 'cardCdNm', width:70 });
    colModel.push({ label: '할부개월', name: 'monthInst', width:60 });
    colModel.push({ label: '원거래 TID', name: 'orgTid', width:220 });
    colModel.push({ label: '취소번호', name: 'cancelNo', width:70 });
    ...
    colModel.push({ label: '카드사', name: 'cardCd', hidden:true });
    ...
    gridProp.loadComplete = function() {
        ...
        for (var i = 0; i < gridData.length; i++) {
            ...
            if (gridData[i].monthInst.length > 0) {
                if (gridData[i].monthInst == '00') {
                    $("#grid1").jqGrid('setRowData', ids[i], {monthInst:"일시불"});
                } else {
                    var monVal = parseInt(gridData[i].monthInst) + "개월";
                    $("#grid1").jqGrid('setRowData', ids[i], {monthInst:monVal});
                }
            }
    ...
    //일자별 PG사 정산대사 수집 데이터 리스트 그리드 생성
...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/work/pgsettledata/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus(  ksid.string.replace( "일자별 PG사 정산대사 수집 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
/*****************************************************
* 함수명 : 일자별 PG사 정산대사 수집 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/work/pgsettledata/excel";
    var fileNm = "일자별PG정산대사수집내역";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">작업관리</span> > <span name="menu">일자별 PG사 정산대사 수집 내역</span></p>
    </div>
    ...
    <h3 class="style-title"><span name="title">일자별 PG사 정산대사 수집 데이터 목록(지급기준:7영업일 후)</span></h3>
    <!-- grid -->
