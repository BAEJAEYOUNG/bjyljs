...
// PG 결제취소 처리 데이터정보 json
var newJson1 = {};

// PG 결제취소 처리 데이터리스트 grid
var grid1 = null;
...
function init() {
    ...
    var colModel = [];
    colModel.push({ label:'취소일시', name:'udrDtm', format:'dttm', width:120 });
    ...
    colModel.push({ label:'취소명령', name:'payCancelCmdNm', width:70});
    colModel.push({ label:'취소결과', name:'rstResultMsg', width:100 });
    //colModel.push({ label:'에러코드', name:'rstErrorCd', width:100 });
    colModel.push({ label:'에러메시지', name:'rstErrorMsg', width:100 });
    colModel.push({ label:'취소금액', name:'rstCancelAmt', format:'number', width:70 });
    colModel.push({ label:'취소일시', name:'rstCancelDtm', format:'dttm', width:120 });
    colModel.push({ label:'결제수단', name:'rstCancelMethodNm', width:70 });
    colModel.push({ label:'가맹점 ID', name:'rstCancelMid', width:80 });
    colModel.push({ label:'거래ID', name:'rstCancelTid', width:220 });
    colModel.push({ label:'거래상태코드', name:'rstStateCdNm', width:80 });
    colModel.push({ label:'부분취소', name:'rstPartialFgNm', width:80 });
    //colModel.push({ label:'승인날짜', name: 'rstAuthDtm', format: 'dttm', width:120 });
    colModel.push({ label:'승인번호', name: 'rstCancelNo', width:70 });
    //colModel.push({ label:'프로모션', name: 'rstPromCd', width:70 });
    //colModel.push({ label:'할인금액', name: 'rstDiscountAmt', format: 'number', width:60 });
    //colModel.push({ label:'혜택복원여부', name: 'rstRecoverFgNm', width:70 });
    //colModel.push({ label:'매입취소구분', name: 'rstCancelCdNm', width:70 });
    ...
    colModel.push({ label:'취소명령', name:'payCancelCmd', hidden:true});
    colModel.push({ label:'결제결과', name:'rstResultCd', hidden:true});
    colModel.push({ label:'결제수단', name:'rstCancelMethod', hidden:true});
    colModel.push({ label:'거래상태코드', name:'rstStateCd', hidden:true});
    colModel.push({ label:'혜택복원여부', name:'rstRecoverFg', hidden:true});
    colModel.push({ label:'매입취소구분', name:'rstCancelCd', hidden:true});
    ...
    gridProp.loadComplete = function() {
        ...
        for (var i = 0; i < gridData.length; i++) {
            //20180308-가상계좌(2211)
            if (gridData[i].rstResultCd == '2001' || gridData[i].rstResultCd == '2002' || gridData[i].rstResultCd == '2211') {
...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/work/pgcanceldata/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus( ksid.string.replace( "PG 결제취소 처리 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
/*****************************************************
* 함수명 : PG 결제취소 처리 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/work/pgcanceldata/excel";
    var fileNm = "PG결제취소처리내역";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">작업관리</span> > <span name="menu">PG 결제취소 처리 내역</span></p>
    </div>
    ...
    <div id="search-panel" class="search-panel">
        <div class="styleDlTable">
            ...
            <dl>
                ...
                <dt>취소결과</dt>
                <dd>
                     <select name="rstResultCd" title="취소결과" class="style-select width110" overall="전체" codeGroupCd="PAY_RESULT" selected_value=""></select>
                </dd>
                <dt>결제수단</dt>
                <dd>
    ...
    <h3 class="style-title"><span name="title">PG 결제취소 처리 데이터 목록</span></h3>
    <!-- grid -->
