...
// PG 결제승인 처리 데이터정보 json
var newJson1 = {};

// PG 결제승인 처리 데이터리스트 grid
var grid1 = null;
...
function init() {
    ...
    //colModel.push({ label:'카드BIN', name:'rstCardBin', width:60 });
    //colModel.push({ label:'카드사포인트', name:'rstCardPt', width:60 });
    ...
    colModel.push({ label:'현금영수증 타입', name:'rstRcptTp', width:100 });
    colModel.push({ label:'현금영수증 승인번호', name:'rstRcptAuthCd', width:120 });
    colModel.push({ label:'현금영수증 TID', name:'rstRcptTid', width:100 });
    //colModel.push({ label:'가상결제은행코드', name:'rstVbankCd', width:60 });
    //colModel.push({ label:'가상결제은행명', name:'rstVbankNm', width:60 });
    //colModel.push({ label:'가상계좌번호', name:'rstVbankNo', width:60 });
    //colModel.push({ label:'입금만료예정일시', name:'rstVbankExpDtm', width:60 });
    //colModel.push({ label:'이통사구분', name:'rstCarrier', width:60 });
    //colModel.push({ label:'휴대폰번호', name:'rstDstAddr', width:60 });
...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/work/pgapprdata/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus( ksid.string.replace( "PG 결제승인 처리 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
/*****************************************************
* 함수명 : PG 결제승인 처리 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/work/pgapprdata/excel";
    var fileNm = "PG결제승인처리내역";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">작업관리</span> > <span name="menu">PG 결제승인 처리 내역</span></p>
    </div>
    ...
    <div id="search-panel" class="search-panel">
        <div class="styleDlTable">
            ...
            <dl>
                ...
                <dt>승인결과</dt>
                <dd>
                     <select name="rstResultCd" title="승인결과" class="style-select width110" overall="전체" codeGroupCd="PAY_RESULT" selected_value=""></select>
                </dd>
                <dt>결제수단</dt>
                <dd>
    ...
    <h3 class="style-title"><span name="title">PG 결제승인 처리 데이터 목록</span></h3>
    <!-- grid -->
