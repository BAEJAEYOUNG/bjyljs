...
function init() {
    ...
    colModel.push({ label: '인증수단', name: 'authMethodNm', width:90 });
...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/bill/mstudr/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus(  ksid.string.replace( "FIDO 과금 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
/*****************************************************
* 함수명 : FIDO 과금 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/bill/mstudr/excel";
    var fileNm = "FIDO과금데이터";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">과금조회</span> > <span name="menu">FIDO 과금 데이터</span></p>
    </div>
...
<!--                     <select name="eYymm" title="종료월" class="style-select" codeGroupCd="yyyy_mm"></select> -->
    ...
    <h3 class="style-title"><span name="title">FIDO 과금 데이터 목록</span></h3>
    <!-- grid -->
