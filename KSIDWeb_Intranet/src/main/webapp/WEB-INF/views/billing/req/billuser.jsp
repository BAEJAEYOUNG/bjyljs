...
function doQuery(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/req/billuser/list"
                      , params
                      , function(result) {

        grid2.initGridData();
        CommonJs.setStatus(  ksid.string.replace( "종량요금 개인 청구서 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
...
function doQuery2(resultMsg) {
    ...
    params.telNo = grid1.clickedRowData.telNo;

    // get_admin_list.do 경로로 params 파라미터로 조회하여 받아온 데이터를 grid1에 바인딩한다.
    ksid.net.ajaxJqGrid(grid2
                      , "${pageContext.request.contextPath}/billing/req/billuser/listDtl"
                      , params
                      , function(result) {

        CommonJs.setStatus(  ksid.string.replace( "종량요금 개인 상세 청구서 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
...
function doExcel() {
    ...
    var url = "${pageContext.request.contextPath}/billing/req/billuser/excel2";
    var fileNm = "종량요금개인청구서";
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">청구관리</span> > <span name="menu">종량요금 개인 청구서</span></p>
    </div>
...
<!--     <h3 class="style-title"><span name="title">종량요금 개인 청구서</span></h3> -->
    <div class="style-title-wrap">
        <h3 class="style-title">종량요금 개인 청구서</h3>
        <div class="button-bar">
    ...
    <!--// grid -->
    <div class="style-title-wrap">
        <h3 class="style-title">종량요금 개인 상세 청구서</h3>
        <div class="button-bar">
