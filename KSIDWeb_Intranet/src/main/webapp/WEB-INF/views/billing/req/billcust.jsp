...
function init() {
    ...
    colModel.push({ label: '부가세', name: 'vatFee', format:'number' ,width:100 });
...
function doQuery(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/req/billcust/list"
                      , params
                      , function(result) {

        grid2.initGridData();
        CommonJs.setStatus(  ksid.string.replace( "종량요금 고객사 청구서 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
...
function doQuery2(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid2
                      , "${pageContext.request.contextPath}/billing/req/billcust/listDtl"
                      , params
                      , function(result) {

        CommonJs.setStatus(  ksid.string.replace( "종량요금 고객사 상세 청구서 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
...
function doExcel() {
    ...
    var url = "${pageContext.request.contextPath}/billing/req/billcust/excel2";
    var fileNm = "종량요금고객사청구서";
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">청구관리</span> > <span name="menu">종량요금 고객사 청구서</span></p>
    </div>
    ...
    <div class="style-title-wrap">
        <h3 class="style-title">종량요금 고객사 청구서</h3>
        <div class="button-bar">
    ...
    <!--// grid -->
    <div class="style-title-wrap">
        <h3 class="style-title">종량요금 고객사 상세 청구서</h3>
        <div class="button-bar">
