...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/settle/setpgtradehis/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus(  ksid.string.replace( "PG거래대사 비교 내역 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
    });
...
/*****************************************************
* 함수명 : PG거래대사 비교 내역 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/settle/setpgtradehis/excel";
    var fileNm = "PG거래대사 비교 내역 데이터목록";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">정산관리</span> > <span name="menu">PG거래대사 비교 내역</span></p>
    </div>
    ...
    <h3 class="style-title"><span name="title">PG거래대사 비교 내역 데이터 목록</span></h3>
    <!-- grid -->
