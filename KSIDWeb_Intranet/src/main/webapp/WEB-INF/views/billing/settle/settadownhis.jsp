...
// TA Download 정산 비교 내역 json
var newJson1 = {};

// TA Download 정산 비교 내역grid
var grid1 = null;
...
function init() {
    ...
    colModel.push({ label: '수집일시', name: 'udrDtm', format:'dttm', width:120});
    ...
    colModel.push({ label: '수집시스템', name: 'sysId', width:80 });
    colModel.push({ label: 'UUID', name: 'uuid', format: 'string', width:250 });
    colModel.push({ label: 'SUID', name: 'suid', format: 'string', width:500 });
    ...
    //TA Download 정산 비교 데이터리스트 그리드 생성
...
/*****************************************************
* 함수명 : TA Download 정산 비교 데이터 목록 조회
* 설명   :
*****************************************************/
...
function doQueryPaging(resultMsg) {
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/billing/settle/settadownhis/list"
                      , params
                      , function(result) {
        ...
        if(resultMsg) {
            ...
            CommonJs.setStatus(  ksid.string.replace( "TA Download 정산 비교 데이터 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
/*****************************************************
* 함수명 : TA Download 정산 비교 데이터목록 엑셀다운로드
* 설명   :
*****************************************************/
function doExcel() {

    var url = "${pageContext.request.contextPath}/billing/settle/settadownhis/excel";
    var fileNm = "TA Download 정산 비교 데이터목록";
    var params = ksid.form.flushPanel("search-panel");
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">정산관리</span> > <span name="menu">TA Download 정산 비교 내역</span></p>
    </div>
    ...
    <h3 class="style-title"><span name="title">TA Download 정산 비교 데이터 목록</span></h3>
    <!-- grid -->
