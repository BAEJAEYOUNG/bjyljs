...
//    colModel.push({label:"상품코드", name:"prodMclsCd", width:80});
    colModel.push({label:"상품명", name:"prodMclsNm", width:120, hidden:true});
    colModel.push({label:"사용자수", name:"userCntP", format:"number", width:100});
    colModel.push({label:"승인건수", name:"apprCntP", format:"number", width:100});
//    colModel.push({label:"승인요금", name:"apprAmtP", format:"number", width:100});
    colModel.push({label:"취소건수", name:"cancelCntP", format:"number", width:100});
//    colModel.push({label:"취소요금", name:"cancelAmtP", format:"number", width:100});
    colModel.push({label:"합계건수", name:"billCntP", format:"number", width:100});
//    colModel.push({label:"합계요금", name:"billFeeP", format:"number", width:100})
    ...
    gridProp.loadComplete = function() {
        ...
        var option = {};
        option.col = [ "apprCntP", "cancelCntP",
                       "userCntP", "billCntP" ];
        ...
        for (var i = 0; i < gridData.length; i++) {
            ...
            if (gridData[i].cancelCntP > 0) {
                $('#grid1').jqGrid('setCell', ids[i], 'cancelCntP', '', cssOrange);
            }
        }
    ...
    group.groupHeaders = [];
    group.groupHeaders.push({startColumnName: 'userCntP', numberOfColumns: 7, titleText: "선불 결제"});
...
function searchpanelComboAjaxAfterDoQuery() {
    ...
    var params = {spCd:"${sessionScope.sessionUser.spCd}"};
...
function doQuery() {
 ...
 // search-panel 의 form name, value를 <k,v> 형태로 params 에 담는다.
    ...
    ksid.net.ajaxJqGrid(grid1
                      , "${pageContext.request.contextPath}/sp/stat/salestatday/list"
                      , params
                      , function(result) {

        if(result.resultCd == '00') {
            setGridHeader();
            CommonJs.setStatus(  ksid.string.replace( "일별 결제현황 목록 <strong>{0}</strong>건이 조회 되었습니다.", "{0}", result.resultData.length ) );
        }
...
function doExcel() {
    ...
    var url = "${pageContext.request.contextPath}/sp/stat/salestatday/excel";
    var fileNm = "일별결제현황목록";
...
<div class="contents-wrap">
    ...
    <div class="location-wrap">
        <p class="location"><span class="btn_home"></span>&nbsp;> <span name="menu">매출현황</span> > <span name="menu">일별 결제현황</span></p>
    </div>
    ...
    <h3 class="style-title"><span name="title">일별 결제현황 목록</span></h3>
    <!-- grid -->
