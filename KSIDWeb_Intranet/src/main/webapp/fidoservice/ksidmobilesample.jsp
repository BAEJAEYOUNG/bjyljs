...
//FIDO서버 URL(real)
/* _FIDO_SERVER_URL    = "https://fidoserver3.crosscert.com:9443/fido/svc/v2/Interface"; */

// 2017-09-18 추가
/* var _VERIFY_SERVER_URL  = "https://fidoserver3.crosscert.com:9443/fido/svc/v2/Interface"; */
...
function genCCFIDOProtocol(operation, flag) {
    ...
    if (true == _USUtil.OS.isAndroid()) {
        ...
        /* alert("A CCFidoProtocol.id : " + CCFidoProtocol.id); */
    } else if (true == _USUtil.OS.isiOS()) {
        CCFidoProtocol.id = idDomainValue;     // 2017-08-21 학번 정보 와 도메인 정보 결합 정보 입력
        /* alert("I CCFidoProtocol.id : " + CCFidoProtocol.id); */
    }
...
// 2017-09-18 추가
function reqAuthTokenValidate() {

     var authToken      = "<%=authTk%>";
 ...
 // 샘플 테스트를 위한 부분이므로 실 상용 구현시 배제
   ...
   $.post(userStatChkUrl, params, function(rcvdata) {
        if( rcvdata.rstParam.retValue == "00" ) {
            ...
            if (true == _USUtil.OS.isAndroid()) {
                _APPKEY_FIDO = 'FjwmADokKwE5LTs3FSY8KyYu8P7r8/61lIDwtfGllJXy4TMGHQoWGxMCZHZ0fGt9c2B6YGpzcn9mfHh0dHxidj8GfWNie3VhZ31tcWF/d3VnHisGNjo5Gi48K3MhPgkoLygkPjA6HCQ7GzEzBHNu';
                /* _APPKEY_FIDO = 'FjwmADokKwE5LTs3FSY8KyYuiILi+vjo7PqP7fuIk4n2ojMGHQoWGxMCZHB3emp3c2p8YWN2dn9mfHh0dXdicT8UfXdiZ3V2GCgcJjwyFh01OisFChYpGAA1MXAvBSdyACEVJG5+'; */
                /* _APPKEY_FIDO = 'FjwmADokKwE5LTs3FSY8KyYu8P7r8/61lIDwtfGllJU/BwEaAAoCHHl5eHN8emJ1cmp7YWJud2Jle3l7d3wvEXFifmRicnZicHptcmF+dwknByoqLywIOhYtMDMyDzE9LwElHhoKPxUdLRh+eA=='; */
            } else if (true == _USUtil.OS.isiOS()) {
                _APPKEY_FIDO = 'FjwmADokKwE5LTs3FSY8KyYu8P7r8/61lIDwtfGllJXy4TMGHQoWGxMCZHZ0fGt9c2B6YGpzcn9mfHh0dHxidj8GfWNie3VhZ31tcWF/d3VnHisGNjo5Gi48K3MhPgkoLygkPjA6HCQ7GzEzBHNu';
                /* _APPKEY_FIDO = 'FjwmADokKwE5LTs3FSY8KyYuiILi+vjo7PqP7fuIk4n2ojMGHQoWGxMCZHVwfmF8d2p7ZGBxfH9mfHh0dXdicT8UfXdiZ3V2GCgcJjwyZnMEPBtqHis2E2YvMQJ8FBcADQd/JG5+'; */
                /* _APPKEY_FIDO = 'FjwmADokKwE5LTs3FSY8KyYu8P7r8/61lIDwtfGllJU/BwEaAAoCHHl5eHN8emJ1cmp7YWJud2Jle3l7d3wvEXFifmRicnZicHptcmF+dwknByoqLywIOhYtMDMyDzE9LwElHhoKPxUdLRh+eA=='; */
            }
...
<input type="hidden" name="custUserNo" value=""/>
    ...
    <!-- 2017-09-18 추가
    <button type="button" id='authTokenValidate' onclick="reqAuthTokenValidate();">FIDO Validate</button>
     <br />
     <br />
     <iframe name="iFrame_authToken_validate" width="350px;" height="50px;"></iframe>
</form>
</body>
</html>